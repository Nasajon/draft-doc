...
| qualificacao              | string(11)  | Tipo de participante perante a lei (pessoa física, jurídica, etc).                                                  | Sim      | Vazio   | ["fisica", "juridica", "publica_federal", "publica_estadual", "publica_municipal", "cooperativa_credito", "cooperativa_agro", "cooperativa", "financeira", "seguradora_capitalizacao", "corretora_autonoma_seguro", "previdencial_complementar_aberta", "previdencial_complementar_fechada", "economia_mista", "outras_publica_federal", "outros"] |
...
| observacao                | string      | Texto livre do observações sobre o participante.                                                                    | Não      | Vazio   |                                                                                                                                                                                                                                                                                                                                                    |
| propriedades_customizadas | json        | JSON de propriedades customizadas pelo cliente.                                                                     | Não      | Vazio   |                                                                                                                                                                                                                                                                                                                                                    |
| site                      | string(150) | Endereço eletrônico do participante.                                                                                | Não      | Vazio   |                                                                                                                                                                                                                                                                                                                                                    |
| inativo                   | boolean     | Indica que um participante está inativo (não excluído, apenas oculo por opção do usuário).                          | Sim      | False   |                                                                                                                                                                                                                                                                                                                                                    |
| cane_principal            | string(7)   | Classificação Nacional de Atividade Econômica principal.                                                            | Não      | False   |                                                                                                                                                                                                                                                                                                                                                    |
...
| empresa_matriz            | uuid        | ID da empresa matriz à qual este participante está ligado como filial.                                              |          |
...
| participante_destino    | uuid    | ID do participante destino do relacionamento (por exemplo, para um relacionamento com a flag "cliente" marcada, o destino está no contratado, ou fornecedor). | Sim      | Vazio   |         |
...
| transportadora          | boolean | Indica que o participante_origem é Transportadora do participante_destino.                                                                                    | Sim      | False   |         |
| representante_comercial | boolean | Indica que o participante_origem é Representante Comercial do participante_destino.                                                                           | Sim      | False   |         |
| representante_tecnico   | boolean | Indica que o participante_origem é Representante Técnico do participante_destino.                                                                             | Sim      | False   |         |
| contato                 | boolean | Indica que o participante_origem é um Contato do participante_destino.                                                                                        | Sim      | False   |         |

_Obs. 1: Para os relacionamentos do tipo `lead, Cliente, Fornecedor, Técnico, Vendedor, Transportador, Representante Comercial e Representante Técnico`, esta tabela de relacionamento será normalmente usada para indicar o relacionamento entre um Participante que figure como uma empresa de um dos grupos empresariais do sistema (marcado com a flag `empresa_membro` como true), e outro participante qualquer (geralmente com a flag `empresa_membro` como false). Nestes casos, a coluna `participante_destino` sempre será preenchida com um participante que também seja uma `empresa_membro` de um Grupo Empresarial. Embora seja possível, não se planeja guardar o relacionamento (de cliente, fornecedor, etc) entre participantes onde ambos não sejam membros de um dos grupos empresariais do tenant._

_Obs. 2: Para os relacionamentos do tipo `Contato`, esta tabela de relacionamento será normalmente usada para indicar o relacionamento entre dois Participantes que não figuram como empresas membro de um dos grupos empresariais do sistema (marcados com a flag `empresa_membro` como false). Também pode-se gravar os contatos de uma `empresa_membro`, mas não se planeja esse como uso normal._
...
| inscricao_estadual | string(20) | Inscrição municipal do participante (no município em questão).                                    | Vazio    | Vazio   |         |
...
| email        | string(150) | Endereço de e-mail.                                                                                                                 | Sim      | Vazio   |         |
| principal    | boolean     | Indica se este é o e-mail principal do participante.                                                                                | Sim      | False   |         |
| financeiro   | boolean     | Indica se é um e-mail para contato de natureza financeira (normalmente útil para e-mails de contato não personificado).             | Não      | Vazio   |
| faturamento  | boolean     | Indica se é um e-mail para contato de natureza fiscal, de faturamento (normalmente útil para e-mails de contato não personificado). | Não      | Vazio   |
| comercial    | boolean     | Indica se é um e-mail para contato de natureza comercial (normalmente útil para e-mails de contato não personificado).              | Não      | Vazio   |
